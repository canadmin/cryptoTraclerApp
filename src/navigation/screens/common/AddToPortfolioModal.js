...
import { addAssetToPortfolio, getAllPortfolio,getTransaction } from "../../../storage/allSchema";
...
const AddToPortfolioModal = (props) => {
  const {
    ...
    isUpdate = false,
    showCoinSearch,
    setPortfolioUpdateDetectionFlag,
    modalInPortfolio,
    navigation,
    headerText
  } = props;
  ...
  const [amount, setAmount] = useState( (coin && coin.id && coin.coinId) ? coin.amount : 1);
  ...
  useEffect(() => {
    if(coin && coin.id && coin.coinId){
      getTransaction(coin.id).then((res) => {
  ...
  const addAsset = async () => {
    await addAssetToPortfolio({
      id:(selectedCoin.id && selectedCoin.coinId) ? selectedCoin.id:null,
      portfolioId: 1,
  ...
  return (
    <Modal
      ...
      <SafeAreaView style={styles.safeAreaStyle}>
        <Header isModal={true} headerText={headerText} setShowModal={setShowModal}> </Header>
        <View style={styles.modal}>
          ...
          <Text
            style={styles.convertDisclaimerText}>{amount} {selectedCoin !== null ? selectedCoin.symbol.toUpperCase() : ""} = {selectedCoin !== null ? priceFormat(amount * coinPrice) : ""}</Text>
          ...
          <View style={styles.rowStyle}>
            ...
            <TouchableOpacity disabled={coin && coin.coinId && coin.id} onPress={() => {
          ...
          <TouchableOpacity onPress={() => addAsset()} style={styles.rowStyle}>
            <View style={styles.addButtonStyle}>
              <Text style={styles.addButtonTextStyle}> {isUpdate ? `Update` : `Create Transaction`}</Text>
            </View>
          </TouchableOpacity>
        </View>
